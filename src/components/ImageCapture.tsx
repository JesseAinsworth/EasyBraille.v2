...
export function ImageCapture({ onTextDetected }: ImageCaptureProps) {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setCapturedImage(imageDataUrl)
        processImageFromFile(file) // Usar el archivo directamente
      }
      reader.readAsDataURL(file)
    }
  }

  // Función para procesar archivo directamente (más eficiente)
  const processImageFromFile = async (file: File) => {
    ...
    try {
      ...
      setDetectedText(data.text) // Tu API devuelve "text", no "translation"
      onTextDetected(data.text)
      toast({ 
        title: "Imagen procesada", 
        description: "Texto detectado exitosamente." 
      })
    } catch (error) {
      console.error("Error processing image:", error)
      toast({ 
        title: "Error", 
        description: `No se pudo procesar la imagen: ${error instanceof Error ? error.message : 'Error desconocido'}`, 
        variant: "destructive" 
      })
    } finally {
  ...
  // Función para procesar desde captura de cámara
  const processImage = async (imageDataUrl: string) => {
    ...
    try {
      ...
      const apiResponse = await fetch("http://localhost:5000/api/braille-image", {
        method: "POST",
        body: formData,
      })
    ...
    try {
      await fetch("http://localhost:5000/api/translations", {
        ...
        body: JSON.stringify({
          ...
          translationType: "braille-to-text",
        }),
      })
      ...
      {detectedText && (
        <div className="mt-4 space-y-3 bg-white border rounded-lg p-4 shadow-md">
          ...
          <div className="flex gap-3 mt-2">
            <Button onClick={handleConfirmTranslation} className="bg-green-600 hover:bg-green-700 text-white">
              Confirmar
            </Button>
            <Button onClick={() => setIsCorrectionOpen(true)} className="bg-yellow-500 hover:bg-yellow-600 text-white">
              Corregir
            </Button>
          </div>
          ...
          {isCorrectionOpen && (
            <div className="mt-2 space-y-2">
              ...
              <div className="flex gap-2">
                <Button onClick={handleSubmitCorrection} className="bg-blue-600 text-white hover:bg-blue-700">
                  Guardar corrección
                </Button>
                <Button onClick={() => setIsCorrectionOpen(false)} variant="outline">
                  Cancelar
                </Button>
              </div>
            </div>
